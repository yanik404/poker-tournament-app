...
interface Props { players: Player[]; teams: Team[]; onChange: (teams: Team[]) => void; }
export function TeamAssignments({ players, teams, onChange }: Props) {
  const move = (playerId: string, destinationId: string) => {
    ...
    if (destination.memberIds.length >= (destination.maxMembers ?? 2)) {
  ...
  return <div className="assignment-list">{players.map(player => { const current = teams.find(team => team.memberIds.includes(player.id)); return <label key={player.id}><span>{player.name}</span><select aria-label={`Team von ${player.name}`} value={current?.id ?? ''} onChange={event => move(player.id, event.target.value)}>{teams.map(team => <option key={team.id} value={team.id}>{team.name} · {team.memberIds.length}/{team.maxMembers ?? 2}</option>)}</select></label>; })}</div>;
}
