...
export function TournamentPlan({ settings, players, stack, levels, onStart, onBack }: { settings: Settings; players: Player[]; stack: ChipStack; levels: Level[]; onStart: (finance: FinanceSettings, teams: Team[]) => void; onBack: () => void }) {
  ...
  return <main className="screen plan">
    ...
    <section className="panel"><h2>Turnierkasse</h2>
      ...
      <h3>Preisgeld · Hauptpot {money(finance.buyIn * players.length)}</h3><div className="prize-editor">{([0, 1, 2] as const).map(index => <label key={index}>{index + 1}. Platz<input aria-label={`${index + 1}. Platz Prozent`} type="number" min="0" max="100" value={finance.prizePercentages[index]} onChange={event => updatePrize(index, Number(event.target.value))}/><span>% · {money(prizes[index])}</span></label>)}</div>{!prizesValid && <p className="validation">Die drei Anteile müssen genau 100 % ergeben und absteigend sein.</p>}
    </section>
