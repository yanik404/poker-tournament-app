...
import { CHIP_COLOURS, DENOMINATIONS, stackValue, totalUsage } from '../logic/chips';
...
export function TournamentPlan({ settings, players, stack, levels, onStart, onBack }: { settings: Settings; players: Player[]; stack: ChipStack; levels: Level[]; onStart: (finance: FinanceSettings, teams: Team[]) => void; onBack: () => void }) {
  ...
  return <main className="screen plan">
    ...
    <section className="panel"><h2>Startstack pro Spieler</h2><div className="stack-value">{stackValue(stack).toLocaleString('de-CH')} <small>Chips</small></div><p className="hint">Optimiert für {settings.players} Spieler · {Math.round(settings.totalMinutes / 60 * 10) / 10} Std. · möglichst viele sinnvolle Chips mit Reserve für Wechseln und Color-Up.</p><div className="chip-grid">{DENOMINATIONS.map(chip => <div key={chip}><b className={`chip chip-${chip}`} style={{ background: CHIP_COLOURS[chip].hex, color: CHIP_COLOURS[chip].text }}>{chip}</b><span>× {stack[chip]}<small>{CHIP_COLOURS[chip].name}</small></span></div>)}</div><h3>Gesamtverbrauch · {settings.players} Spieler</h3><div className="usage">{DENOMINATIONS.map(chip => <span key={chip}><i className={`usage-dot chip-${chip}`} style={{ background: CHIP_COLOURS[chip].hex }}/>{chip}: <b>{use[chip]}</b>/ {chip === 500 || chip === 1000 ? 50 : 100}</span>)}</div></section>
