...
import { createTeams, DEFAULT_FINANCE, mainPrizes, money, playerContribution, teamShare } from '../logic/finances';
...
export function TournamentPlan({ settings, players, stack, levels, onStart, onBack }: { settings: Settings; players: Player[]; stack: ChipStack; levels: Level[]; onStart: (finance: FinanceSettings, teams: Team[]) => void; onBack: () => void }) {
  ...
  const setTeamSize = (size: 0 | 2 | 3) => { setFinance(old => ({ ...old, teamSize: size })); setTeams(createTeams(players, size)); };
  return <main className="screen plan">
    ...
    <section className="panel"><h2>Turnierkasse</h2>
      ...
      <label className="select-label">Team-Modus<select aria-label="Team-Modus" value={finance.teamSize} onChange={event => setTeamSize(Number(event.target.value) as 0 | 2 | 3)}><option value={0}>Kein Team-Pot</option><option value={2}>2er-Teams</option><option value={3}>3er-Teams</option></select></label>
      {finance.teamSize > 0 && <><label className="money-input">Team-Pot pro Team<input aria-label="Team-Pot pro Team" type="number" min="0" step="1" value={finance.teamPot} onChange={event => updateMoney('teamPot', Number(event.target.value))}/><b>CHF</b></label><p className="hint">Ordne hier jede Person dem gewünschten Team zu. Ein Team kann höchstens {finance.teamSize} Mitglieder haben.</p><TeamAssignments players={players} teams={teams} teamSize={finance.teamSize as 2 | 3} onChange={setTeams}/><div className="team-list">{teams.map(team => <div key={team.id}><b>{team.name}</b><span>{team.memberIds.map(id => players.find(player => player.id === id)?.name).join(' · ')}</span><small>{money(teamShare(team, finance))} pro Person</small></div>)}</div></>}
      ...
      <h3>Preisgeld · Hauptpot {money(finance.buyIn * players.length)}</h3><div className="prizes"><span>1. {money(prizes[0])}</span><span>2. {money(prizes[1])}</span><span>3. {money(prizes[2])}</span></div>
    </section>
    ...
    <button className="primary" onClick={() => onStart(finance, teams)}>Turnier starten</button>
  </main>;
}
